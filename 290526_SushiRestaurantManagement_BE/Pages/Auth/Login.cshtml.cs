...
namespace _290526_SushiRestaurantManagement_BE.Pages.Auth
{
    public class LoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            // 1. KI?M TRA TÀI KHO?N ADMIN FIX C?NG TR??C
            ...
            if (phone == adminPhone && Input.Password == adminPassword)
            {
                ...
                HttpContext.Session.SetString("StaffRole", "Admin"); // ?ánh d?u vai trò

                return RedirectToPage("/Admin/Index"); // ?i?u h??ng th?ng vào Dashboard Admin
            }

            // 2. N?U KHÔNG PH?I ADMIN THÌ KI?M TRA STAFF TRONG DATABASE NH? C?
            var staff = await _context.Staffs
            ...
            HttpContext.Session.SetString("StaffName", staff.FullName ?? "");
            HttpContext.Session.SetString("StaffPhone", staff.Phone ?? "");
            HttpContext.Session.SetString("StaffRole", "Staff"); // Tài kho?n nhân viên thông th??ng
